...
export class QuestionScene extends Phaser.Scene {
  ...
  private createQuestionAssistPanel(): void {
    ...
    this.trackAssistObject(this.add.text(sx(46), sy(298), "현재 질문", {
      fontFamily: "system-ui, sans-serif",
      fontSize: `${ss(14)}px`,
      color: "#f6d365",
      fontStyle: "bold",
    ...
    this.questionPreviewText = this.trackAssistObject(this.add.text(sx(46), sy(334), "", {
      fontFamily: "system-ui, sans-serif",
      fontSize: `${ss(14)}px`,
      color: "#f8f0ff",
      lineSpacing: ss(5),
  ...
  private updateNextButtonVisibility(): void {
    const visible = this.shouldShowNextButton();
    this.nextButtonBg?.setVisible(visible);
    this.nextButtonLabel?.setVisible(visible);
    this.nextButtonHitZone?.setVisible(visible);
  }
  ...
  private handlePrimaryAction(): void {
    if (!this.shouldShowNextButton()) return;
  ...
  private setPhase(phase: QuestionPhase): void {
    ...
    if (isQuestionPhase) {
      ...
      this.phaseGuideText?.setText("2 / 3\n선택지를 눌러 질문을 한 번 이상 다듬으면 다음 버튼이 열립니다.");
