...
export class ChatScene extends Phaser.Scene {
  ...
  init(data: ChatSceneData): void {
    ...
    this.messages = [
      {
        role: "assistant",
        content: data.reading.npcLine || data.reading.summary,
      },
    ];
  }
  ...
  private createChatDom(): void {
    ...
    shell.innerHTML = `
      ...
      <form class="arcana-chat-form" data-chat-form>
        <textarea class="arcana-chat-input" data-chat-input maxlength="500" rows="1" placeholder="예: 지금 내가 가장 먼저 해야 할 일은?"></textarea>
        <button class="arcana-button" type="submit">전송</button>
      </form>
    `;
  ...
  private async sendMessage(): Promise<void> {
    ...
    this.renderMessages("별빛을 다시 읽는 중...");
    this.isSending = true;
    ...
    try {
      ...
      this.messages.push({
        role: "assistant",
        content: "안개가 잠시 짙어졌습니다. 조금 뒤에 다시 물어보세요.",
      });
    } finally {
