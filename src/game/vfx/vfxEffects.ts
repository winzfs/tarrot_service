...
export function addSoftGlow(scene: Phaser.Scene, x: number, y: number, depth: number, scale = 1): VfxObject {
  ...
  return glow ?? scene.add.circle(x, y, ss(64) * scale, 0x6d4aff, 0).setDepth(depth);
}
...
export function addRuneRing(scene: Phaser.Scene, x: number, y: number, depth: number, scale = 1): VfxObject {
  ...
  return ring ?? scene.add.circle(x, y, ss(54) * scale, 0xf6d365, 0).setDepth(depth).setStrokeStyle(ss(3), 0xf6d365, 0.85);
}
...
export function createMagicBeam(scene: Phaser.Scene, fromX: number, fromY: number, toX: number, toY: number, depth: number, tint = 0xf6d365): VfxObject {
  ...
  if (textureReady(scene, beamAsset)) {
    return scene.add.image(fromX, fromY, beamAsset.key).setOrigin(0, 0.5).setRotation(angle).setDisplaySize(distance, ss(38)).setAlpha(0).setDepth(depth).setBlendMode(Phaser.BlendModes.ADD).setTint(tint);
  }
...
export function playBurst(scene: Phaser.Scene, x: number, y: number, depth: number, scale = 1): void {
  ...
  if (burst) {
    scene.tweens.add({ targets: burst, alpha: 0.95, scale: 1.45 * scale, duration: 260, ease: "Cubic.easeOut", onComplete: () => scene.tweens.add({ targets: burst, alpha: 0, scale: 1.9 * scale, duration: 460, ease: "Sine.easeOut", onComplete: () => burst.destroy() }) });
    return;
  }
...
export function playSmoke(scene: Phaser.Scene, x: number, y: number, depth: number, scale = 1): void {
  ...
  smokeAssets.slice(0, 2).forEach((asset, index) => {
    ...
    if (!smoke) return;
    scene.tweens.add({ targets: smoke, alpha: 0.32, scale: (0.98 + index * 0.2) * scale, x: smoke.x + (index === 0 ? -ss(28) : ss(32)), y: smoke.y - ss(18 + index * 8), angle: index === 0 ? -16 : 18, duration: 1150, ease: "Sine.easeInOut", onComplete: () => scene.tweens.add({ targets: smoke, alpha: 0, duration: 520, ease: "Sine.easeOut", onComplete: () => smoke.destroy() }) });
  });
}
