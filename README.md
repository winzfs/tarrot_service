...
현재 코드 기준의 씬 구성, API, 카드 이미지, VFX, 배열법, 배포 상태, 다음 작업을 한눈에 정리합니다.
...
- 공개 시 발광, 파동, 파티클 효과
- 카드 수별 배치: 1장 중앙, 3장 가로, 5장 위 2장 + 아래 3장
- 카드명은 한글명과 영어명을 분리해 표시

### 리딩 화면

- 배열 위치에 따른 챕터 제목과 위치 의미 표시
- 카드 한 장을 크게 보여준 뒤 점술사 해석 대화창 표시
- 1장/3장/5장 챕터 진행 지원
- 챕터/종장 연출 중 터치 잠금
- 대화창 또는 종장 본문까지 나온 뒤 3초 후 다음 진행 가능
- 종장에서는 선택된 카드들이 융합되고 종합 조언을 문장 단위로 표시

### 후속 대화 화면

- 리딩 맥락을 유지한 채 `/api/chat` 호출
- 배열법, 카드 위치 의미, 카드별 맥락을 후속 대화 프롬프트에 전달
- 점술사 NPC 말투 유지
...
- 관계 배열에서는 상대의 마음을 사실처럼 단정하지 않습니다.
- 선택 배열에서는 한쪽 선택을 운명처럼 강요하지 않습니다.
...
```txt
1. 5장 배열 모바일 실기기 QA
2. 5장 리딩 화면 카드명/대화창/종장 레이아웃 미세 조정
3. 선택된 배열을 사용자가 직접 바꿀 수 있는 UI 추가 검토
4. 정방향/역방향 카드 시스템 추가
5. AI 응답 품질 개선
6. 카드 이미지/VFX 로딩 최적화
7. 사운드 효과 추가
